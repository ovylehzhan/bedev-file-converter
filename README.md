...
- **Duplicated `ConversionJob` entity:** The same entity is defined in both api-gateway and conversion-service (they're currently identical). They must be kept in sync by hand — a field change in one and not the other would cause runtime errors. The clean fix is a shared types package, but that conflicts with the current per-service Docker build context (each service only builds its own folder) and would require a monorepo restructure — deliberately out of scope for this MVP.
